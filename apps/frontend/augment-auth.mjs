...
tokenManagerContent += "    if (this.strategy === 'localStorage' && typeof window !== 'undefined') {\n";
...
tokenManagerContent += "  addRefreshSubscriber(cb: (token: string) => void) { this.refreshQueue.push(cb); }\n";
...
guardsContent += "export function PublicRoute({ children }: { children: React.ReactNode }) { return <>{children}</>; }\n";
guardsContent += "export function ProtectedRoute({ children }: { children: React.ReactNode }) { return <>{children}</>; }\n";
guardsContent += "export function GuestRoute({ children }: { children: React.ReactNode }) { return <>{children}</>; }\n";
guardsContent += "export function RoleRoute({ children, roles }: { children: React.ReactNode, roles: string[] }) { return <>{children}</>; }\n";
guardsContent += "export function PermissionRoute({ children, permission }: { children: React.ReactNode, permission: string }) { return <>{children}</>; }\n";
guardsContent += "export function OrganizationRoute({ children, orgId }: { children: React.ReactNode, orgId: string }) { return <>{children}</>; }\n";
guardsContent += "export function FeatureFlagRoute({ children, flag }: { children: React.ReactNode, flag: string }) { return <>{children}</>; }\n";
fs.writeFileSync(path.join(authDir, 'Guards.ts'), guardsContent);

console.log("Successfully augmented Authentication Layer with Refresh Queue, Configurable Storage, and Route Protection.");
