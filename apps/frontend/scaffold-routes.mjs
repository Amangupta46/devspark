...
const modules = {
  ...
  settings: ['profile', 'organization', 'members', 'roles', 'permissions', 'billing', 'integrations', 'security', 'audit']
};

const boilerplate = {
  layout: (mod) => `export default function ${mod.charAt(0).toUpperCase() + mod.slice(1)}Layout({ children }: { children: React.ReactNode }) {
  return <>{children}</>;
}`,
...
console.log("Successfully scaffolded " + Object.keys(modules).length + " enterprise modules with " + Object.values(modules).flat().length + " sub-routes.");
