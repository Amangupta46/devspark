...
export async function getEstimates(params?: PaginationParams): Promise<PaginatedResponse<Estimate>> {
  const response = await apiClient.get<PaginatedResponse<Estimate>>("/finance/estimates/", { params });
  return response.data;
}
...
export async function getSubscriptions(params?: PaginationParams): Promise<PaginatedResponse<Subscription>> {
  const response = await apiClient.get<PaginatedResponse<Subscription>>("/finance/subscriptions/", { params });
  return response.data;
}
