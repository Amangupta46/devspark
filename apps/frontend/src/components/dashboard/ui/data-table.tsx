...
export function DataTable<T>({ data, columns, onRowClick, keyExtractor }: DataTableProps<T>) {
  ...
  return (
    <div className="w-full overflow-x-auto rounded-xl border border-border-default bg-surface-base">
      <table className="w-full text-left text-sm text-neutral-400">
        <thead className="bg-surface-ground text-xs uppercase text-neutral-500 border-b border-border-default">
          <tr>
            <th scope="col" className="p-4 w-12">
              ...
              >
                {selectedRows.size === data.length && data.length > 0 && <Check className="h-3 w-3" />}
              </div>
            </th>
            {columns.map((col, idx) => (
              <th 
            ...
            {data.map((item, idx) => {
              ...
              return (
                <motion.tr
                  ...
                  className={cn(
                    "border-b border-border-subtle hover:bg-surface-raised transition-colors group",
                ...
                >
                  ...
                  {columns.map((col, colIdx) => (
                    <td key={colIdx} className="px-6 py-4 text-neutral-300 font-medium">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      ...
                      className="inline-flex h-8 w-8 items-center justify-center rounded-md text-neutral-400 hover:bg-surface-floating hover:text-neutral-0 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                    >
