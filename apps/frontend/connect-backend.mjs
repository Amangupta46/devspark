...
const rootDir = path.join(process.cwd(), 'apps', 'frontend', 'apps', 'frontend', 'src', 'app', 'dashboard');
...
const actualDir = fs.existsSync(rootDir) ? rootDir : path.join(process.cwd(), 'apps', 'frontend', 'src', 'app', 'dashboard');
...
allFiles.forEach(filePath => {
  ...
  if (content.includes('const mockData = Array.from')) {
    // 1. Remove mockData
    content = content.replace(/const mockData = Array\.from\(\{ length: 15 \}\)\.map\(\(\_, i\) => \(\{[\s\S]*?\}\)\);\r?\n\r?\n/m, '');
    ...
    content = content.replace(/<AdvancedTable data=\{mockData\}/g, "const { data, isLoading } = useQuery({ queryKey: ['" + mod + "'], queryFn: async () => [] });\n\n        <AdvancedTable data={data || []} isLoading={isLoading}");
    
    // Handle KanbanBoard mock reference
    content = content.replace(/<KanbanBoard data=\{mockData\}/g, "const { data, isLoading } = useQuery({ queryKey: ['" + mod + "'], queryFn: async () => [] });\n\n        <KanbanBoard data={data || []} isLoading={isLoading}");
...
console.log("Successfully connected React Query hooks and eradicated mockData across " + processedCount + " UI screens.");
