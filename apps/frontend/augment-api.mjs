...
features.forEach((mod) => {
  ...
  if (fs.existsSync(typesPath)) {
    ...
    if (!content.includes("RequestDTO")) {
      ...
      content +=
        ...
        "PaginationDTO {\n  page: number;\n  limit: number;\n  total: number;\n}\n";
      content +=
        "export interface " + modCap + "FilterDTO {\n  search?: string;\n  status?: string;\n}\n";
  ...
  if (fs.existsSync(constPath)) {
    ...
    if (!content.includes("Keys = {")) {
      ...
      content +=
        "  list: (filters: string) => [... " + mod + "Keys.lists(), { filters }] as const,\n";
...
console.log(
  "Successfully augmented API layer for " + processedCount + " features non-destructively.",
);
